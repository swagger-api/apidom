...
export class DefaultValidationServiceApidom implements ValidationService {
  ...
  private async validateReferencesConcurrent(
    ...
    try {
      ...
      for (const derefResult of derefResults) {
        const message =
          DefaultValidationServiceApidom.buildReferenceErrorMessageFromResult(derefResult);
        if (message) {
          // @ts-ignore
  ...
  private async validateReferencesSequential(
    ...
    for (const refEl of refElements) {
      ...
      try {
        ...
        const message = DefaultValidationServiceApidom.buildReferenceErrorMessageFromError(ex);
        if (message) {
          // @ts-ignore
          if (refEl as Element) {
