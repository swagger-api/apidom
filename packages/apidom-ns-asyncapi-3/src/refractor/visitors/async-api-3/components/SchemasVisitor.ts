...
import { isReferenceLikeElement, isSchemaElement } from '@swagger-api/apidom-ns-asyncapi-2';
...
class SchemasVisitor extends Mixin(MapVisitor, FallbackVisitor) {
  ...
  declare protected readonly specPath: SpecPath<
    ['document', 'objects', 'Reference'] | ['document', 'objects', 'Schema'] | ['document', 'objects', 'MultiFormatSchema']
  >;

  constructor(options: SchemasVisitorOptions) {
    super(options);
    this.element = new ComponentsSchemasElement();
    this.specPath = (element: unknown) => isReferenceLikeElement(element)
      ? ['document', 'objects', 'Reference']
      : isMultiFormatSchemaElement(element)
  ...
  ObjectElement(objectElement: ObjectElement) {
    ...
    // @ts-ignore
    this.element.filter(isMultiFormatSchemaElement).forEach((multiFormatSchemaElement: MultiFormatSchema) => {
