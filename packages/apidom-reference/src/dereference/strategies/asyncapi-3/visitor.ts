...
class AsyncAPI3DereferenceVisitor {
  ...
  public async ReferenceElement(
    ...
    if (ancestorsLineage.includes(referencedElement)) {
      ...
      if (this.options.dereference.circular === 'error') {
        ...
        const replacer =
          this.options.dereference.strategyOpts['asyncapi-3']?.circularReplacer ??
  ...
  public async ChannelItemElement(
    referencingElement: ChannelElement,
    key: string | number,
    parent: Element | undefined,
    path: (string | number)[],
    ancestors: [Element | Element[]],
    ...
    if (isPrimitiveElement(referencedElement)) {
      ...
      if (this.refractCache.has(cacheKey)) {
        ...
        referencedElement = ChannelElement.refract(referencedElement);
        this.refractCache.set(cacheKey, referencedElement);
      }
    }
    ...
    if (
      (isExternalReference ||
        isNonRootDocument ||
        (isChannelElement(referencedElement) && isStringElement(referencedElement.$ref)) ||
        shouldDetectCircular) &&
      !ancestorsLineage.includesCycle(referencedElement)
    ) {
      // append referencing reference to ancestors lineage
     ...
     */
    if (isChannelElement(referencedElement)) {
      const mergedElement = new ChannelElement(
