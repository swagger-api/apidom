...
import { mediaTypes } from '@swagger-api/apidom-parser-adapter-openapi-json-3-2';
import { fileURLToPath } from 'node:url';

import File from '../../../../src/File.ts';
import OpenAPIJSON3_2Parser from '../../../../src/parse/parsers/openapi-json-3-2/index.ts';

const __dirname = path.dirname(fileURLToPath(import.meta.url));

describe('parsers', function () {
  context('OpenAPIJSON3_2Parser', function () {
    context('canParse', function () {
      context('given file with .json extension', function () {
        context('and with proper media type', function () {
          specify('should return true', async function () {
            ...
            const parser = new OpenAPIJSON3_2Parser();
      ...
      context('given file with unknown extension', function () {
        specify('should return false', async function () {
          ...
          const parser = new OpenAPIJSON3_2Parser({ fileExtensions: ['.json'] });
      ...
      context('given file with supported extension', function () {
        context('and file data is buffer and can be detected as OpenAPI 3.2.0', function () {
          specify('should return true', async function () {
        ...
        context('and file data is string and can be detected as OpenAPI 3.2.0', function () {
          specify('should return true', async function () {
            const url = path.join(__dirname, 'fixtures', 'sample-api.json');
            const file = new File({
              uri: '/path/to/open-api.json',
    ...
    context('parse', function () {
      context('given OpenApi 3.2.x JSON data', function () {
      ...
      context('given OpenApi 3.2.x JSON data as buffer', function () {
        specify('should return parse result', async function () {
          const uri = path.join(__dirname, 'fixtures', 'sample-api.json');
      ...
      context('given data that is not an OpenApi 3.2.x JSON data', function () {
      ...
      context('sourceMap', function () {
        context('given sourceMap enabled', function () {
          specify('should decorate ApiDOM with source maps', async function () {
            ...
            const parser = new OpenAPIJSON3_2Parser({ sourceMap: true });
            const parseResult = await parser.parse(file);
