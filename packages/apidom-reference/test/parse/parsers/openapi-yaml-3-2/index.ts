...
import { mediaTypes } from '@swagger-api/apidom-parser-adapter-openapi-yaml-3-2';
import { fileURLToPath } from 'node:url';

import File from '../../../../src/File.ts';
import OpenAPIYAML3_2Parser from '../../../../src/parse/parsers/openapi-yaml-3-2/index.ts';

const __dirname = path.dirname(fileURLToPath(import.meta.url));

describe('parsers', function () {
  context('OpenAPIYAML3_2Parser', function () {
    context('canParse', function () {
      context('given file with .yaml extension', function () {
        context('and with proper media type', function () {
          specify('should return true', async function () {
            ...
            const parser = new OpenAPIYAML3_2Parser();
      ...
      context('given file with unknown extension', function () {
        specify('should return false', async function () {
          ...
          const parser = new OpenAPIYAML3_2Parser({ fileExtensions: ['.yaml'] });
    ...
    context('parse', function () {
      context('given OpenApi 3.2.x YAML data', function () {
      ...
      context('given OpenApi 3.2.x YAML data as buffer', function () {
        specify('should return parse result', async function () {
          const uri = path.join(__dirname, 'fixtures', 'sample-api.yaml');
      ...
      context('given data that is not an OpenApi 3.2.x YAML data', function () {
      ...
      context('sourceMap', function () {
        context('given sourceMap enabled', function () {
          specify('should decorate ApiDOM with source maps', async function () {
            ...
            const parser = new OpenAPIYAML3_2Parser({ sourceMap: true });
            const parseResult = await parser.parse(file);
