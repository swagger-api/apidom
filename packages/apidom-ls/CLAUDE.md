...
- **AsyncAPI 3.0**: Fields like `messageId` and `schemaFormat` were removed from Message Trait Object
- **AsyncAPI 3.0**: Security Requirement Object was removed; `security` is now an array of Security Scheme Object | Reference Object
- **AsyncAPI 3.0**: Operation `messages` field is an **array** of Message Objects, not a map

**Action**: Before coding, read the specification section at https://www.asyncapi.com/docs/reference/specification/ and verify field existence and types.
...
find packages/apidom-ns-asyncapi-3/src/elements -name "*.ts" | grep -i "ElementName"

# Check if it extends ArrayElement or ObjectElement
grep "class.*ElementName.*extends" packages/apidom-ns-asyncapi-3/src/elements/ElementName.ts
```

**Examples from PR #5104:**
...
- [ ] Check namespace element definition in `packages/apidom-ns-{spec}/src/elements/{Object}.ts`
...
- [ ] Verify element names used in refractor (e.g., `'message'`, `'securityScheme'`)
