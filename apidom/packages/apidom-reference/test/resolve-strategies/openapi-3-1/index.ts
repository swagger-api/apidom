...
import OpenApi3_1ResolveStrategy from '../../../src/resolve-strategies/openapi-3-1';
...
describe('resolve-strategies', function () {
  context('openapi-3-1', function () {
    const strategy = OpenApi3_1ResolveStrategy();
    ...
    context('resolve', function () {
      context('given OpenApi 3.1.x document with single external reference', function () {
        ...
        context('given 1st reference', function () {
          ...
          specify('should have expected resolved value', async function () {
            assert.isTrue(firstReference.value.first?.equals(JSON.parse(rootFile.data.toString())));
          });
        });
      ...
      context('given OpenApi 3.1.x document with depth 4 external references', function () {
        context('and external resolution is turned on', function () {
          let rootFile: IFile;
          let refSet: IReferenceSet;
          ...
          beforeEach(async function () {
            ...
            const data = fs.readFileSync(uri);
            const parseResult = await parse(uri, { parse: { mediaType } });
            const options = mergeOptions(defaultOptions, { resolve: { external: true } });

            rootFile = File({ uri, mediaType, data, parseResult });
            refSet = await strategy.resolve(rootFile, options);
          });
        ...
        context('given external resolution is turned off', function () {
          let rootFile: IFile;
          let refSet: IReferenceSet;
          ...
          beforeEach(async function () {
            ...
            const data = fs.readFileSync(uri);
            const parseResult = await parse(uri, { parse: { mediaType } });
            const options = mergeOptions(defaultOptions, { resolve: { external: false } });
