...
import openApiNamespace, { OpenApi3_2Element } from '@swagger-api/apidom-ns-openapi-3-2';
...
export const detectionRegExp =
  /(?<YAML>^(["']?)openapi\2\s*:\s*(["']?)(?<version_yaml>3\.2\.(?:[1-9]\d*|0))\3(?:\s+|$))|(?<JSON>"openapi"\s*:\s*"(?<version_json>3\.2\.(?:[1-9]\d*|0))")/m;
...
export const parse = async (
  ...
  if (isNotUndefined(result)) {
    const openApiElement = OpenApi3_2Element.refract(result, refractorOpts);
