 ...
 * @public
 */
export interface OpenAPI3_2ResolveStrategyOptions extends Omit<ResolveStrategyOptions, 'name'> {}

/**
 * @public
 */
class OpenAPI3_2ResolveStrategy extends ResolveStrategy {
  constructor(options?: OpenAPI3_2ResolveStrategyOptions) {
  ...
  canResolve(file: File, options: ReferenceOptions): boolean {
    const dereferenceStrategy = options.dereference.strategies.find(
      (strategy) => strategy.name === 'openapi-3-2',
    );
  ...
  async resolve(file: File, options: ReferenceOptions) {
    ...
    if (dereferenceStrategy === undefined) {
      throw new UnmatchedDereferenceStrategyError(
        '"openapi-3-2" dereference strategy is not available.',
      );
    }
...
export default OpenAPI3_2ResolveStrategy;
