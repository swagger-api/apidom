...
import { load as loadYaml } from 'yaml-ast-parser';
  ...
  public async doValidation(
    ...
    if (result.annotations) {
      if (!validationContext?.nativeYamlSyntaxValidation && nameSpace.format === 'YAML') {
        try {
          const astParserResult = loadYaml(text);
          if (astParserResult.errors && astParserResult.errors.length > 0) {
            for (const e of astParserResult.errors) {
              // @ts-ignore
              const endOffset = textDocument.offsetAt({
                line: e.mark.line,
                character: e.mark.column,
              });
              // @ts-ignore
