...
import { CompletionService, CompletionsCollector } from './completion-service';
...
export class DefaultCompletionServiceApidom implements CompletionService {
  ...
  private getMetadataPropertyCompletions(
    ...
    for (const item of filteredCompletions) {
      if (item.function) {
        ...
        if (func) {
          try {
            ...
            if (lintRes) {
              ...
              for (const customItem of customCompletionItems) {
                DefaultCompletionServiceApidom.style(customItem, item, yaml, quotes);
              }
            }
          } catch (e) {
          ...
          item.label = DefaultCompletionServiceApidom.DELETEME;
        }
      } else {
        DefaultCompletionServiceApidom.style(item, item, yaml, quotes);
      }
      delete item.type;
      delete item.format;
    ...
    filteredCompletions = filteredCompletions.filter(
      (ci) => ci.label !== DefaultCompletionServiceApidom.DELETEME,
    );
