...
import { mediaTypes } from '@swagger-api/apidom-parser-adapter-asyncapi-json-3';
import { fileURLToPath } from 'node:url';

import File from '../../../../src/File.ts';
import AsyncAPIJSON3Parser from '../../../../src/parse/parsers/asyncapi-json-3/index.ts';

const __dirname = path.dirname(fileURLToPath(import.meta.url));

describe('parsers', function () {
  context('AsyncAPIJSON3Parser', function () {
    context('canParse', function () {
      context('given file with .json extension', function () {
        context('and with proper media type', function () {
          specify('should return true', async function () {
            ...
            const parser = new AsyncAPIJSON3Parser();
      ...
      context('given file with unknown extension', function () {
        specify('should return false', async function () {
          ...
          const parser = new AsyncAPIJSON3Parser({ fileExtensions: ['.json'] });
    ...
    context('parse', function () {
      context('given AsyncApi 3.0.0 JSON data', function () {
      ...
      context('given AsyncApi 3.0.0 JSON data as buffer', function () {
        specify('should return parse result', async function () {
          const uri = path.join(__dirname, 'fixtures', 'sample-api.json');
      ...
      context('given data that is not an AsyncApi 3.0.0 JSON data', function () {
      ...
      context('sourceMap', function () {
        context('given sourceMap enabled', function () {
          specify('should decorate ApiDOM with source maps', async function () {
            ...
            const parser = new AsyncAPIJSON3Parser({ sourceMap: true });
            const parseResult = await parser.parse(file);
        ...
        context('given sourceMap disabled', function () {
          specify('should not decorate ApiDOM with source maps', async function () {
            ...
            const parser = new AsyncAPIJSON3Parser({ sourceMap: false });
            const parseResult = await parser.parse(file);
