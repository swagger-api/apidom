...
import { mediaTypes } from '@swagger-api/apidom-parser-adapter-asyncapi-yaml-3';
import { fileURLToPath } from 'node:url';

import File from '../../../../src/File.ts';
import AsyncAPIYAML3Parser from '../../../../src/parse/parsers/asyncapi-yaml-3/index.ts';

const __dirname = path.dirname(fileURLToPath(import.meta.url));

describe('parsers', function () {
  context('AsyncAPIYAML3Parser', function () {
    context('canParse', function () {
      context('given file with .yaml extension', function () {
        context('and with proper media type', function () {
          specify('should return true', async function () {
            ...
            const parser = new AsyncAPIYAML3Parser();
      ...
      context('given file with unknown extension', function () {
        specify('should return false', async function () {
          ...
          const parser = new AsyncAPIYAML3Parser({ fileExtensions: ['.yaml', '.yml'] });
      ...
      context('given file with supported extension', function () {
        context('and file data is buffer and can be detected as AsyncAPI 3.0.0', function () {
          specify('should return true', async function () {
        ...
        context('and file data is string and can be detected as AsyncAPI 3.0.0', function () {
          specify('should return true', async function () {
            const url = path.join(__dirname, 'fixtures', 'sample-api.yaml');
            const file = new File({
              uri: '/path/to/async-api.yaml',
    ...
    context('parse', function () {
      context('given AsyncApi 3.0.0 YAML data', function () {
      ...
      context('given AsyncApi 3.0.0 YAML data as buffer', function () {
        specify('should return parse result', async function () {
          const uri = path.join(__dirname, 'fixtures', 'sample-api.yaml');
      ...
      context('given data that is not an AsyncApi 3.0.0 YAML data', function () {
      ...
      context('sourceMap', function () {
        context('given sourceMap enabled', function () {
          specify('should decorate ApiDOM with source maps', async function () {
            ...
            const parser = new AsyncAPIYAML3Parser({ sourceMap: true });
            const parseResult = await parser.parse(file);
        ...
        context('given sourceMap disabled', function () {
          specify('should not decorate ApiDOM with source maps', async function () {
            ...
            const parser = new AsyncAPIYAML3Parser({ sourceMap: false });
            const parseResult = await parser.parse(file);
