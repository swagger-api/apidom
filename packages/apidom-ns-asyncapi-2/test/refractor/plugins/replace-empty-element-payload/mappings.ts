...
/**
 * this test includes test cases related to both `refractorPluginReplaceEmptyElement` and
 * `refractorPluginReplaceEmptyElementPayload` to verify execution of multiple plugins.
 */
describe('given empty value instead of InfoElement', function () {
  it('should replace empty value with semantic element', async function () {
    ...
    const asyncApiElement = AsyncApi2Element.refract(apiDOM.result, {
      plugins: [refractorPluginReplaceEmptyElement(), refractorPluginReplaceEmptyElementPayload()],
    });
...
describe('given empty value instead for message.payload SchemaElement', function () {
  it('should replace empty value with semantic element', async function () {
    const yamlDefinition = dedent`
          asyncapi: 2.2.0
          components:
...
describe('given empty value instead for messageTrait.payload SchemaElement', function () {
  it('should replace empty value with semantic element', async function () {
    const yamlDefinition = dedent`
          asyncapi: 2.2.0
          components:
